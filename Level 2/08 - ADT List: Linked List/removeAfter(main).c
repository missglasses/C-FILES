    ...
    NodePtr removeAfter(Node *head, int item){
        ...
        NodePtr removedList = current->next; /*takesn node after current and removed list points to the 
        node to be deleted
        Before: [current] -> [node_to_remove] -> [next_node] -> NULL
        */
        current->next=NULL; /*cuts off; 
    Before: [current] -> [node_to_remove] -> [next_node] -> NULL
    After:  [current] -> NULL*/
        return removedList;
